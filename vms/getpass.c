...
static char software_version[] = "$Id: getpass.c,v 1.2 2003/05/20 11:34:41 freddy77 Exp $";
...
readpassphrase(const char *prompt, char *pbuf, size_t buflen, int flags) {
  ...
  */
  if ( ! (flags & RPP_TIMEOUT_OFF) ) {
      ...
      if ( ! $VMS_STATUS_SUCCESS(status) ) timeout_secs = DEFAULT_TIMEOUT;
  ...
  if ( !(flags & RPP_ECHO_ON) &&  (ttdevclass == DC$_TERM) ) {
      ...
      */
          ...
          if ( $VMS_STATUS_SUCCESS(status) ) {
              ...
              if ( ! (flags & RPP_TIMEOUT_OFF) )  qio_func |= IO$M_TIMED;
      ...
      */
          ...
          if ( keyboard_id == 0 ) {
             ...
             status = SMG$CREATE_VIRTUAL_KEYBOARD( &keyboard_id, &ttdsc, 
                                                   0, 0, &recall_size );
          }
...
getpass(const char *prompt) {
    ...
    return(
        readpassphrase( prompt, passbuf, sizeof(passbuf), RPP_ECHO_OFF )
    );
